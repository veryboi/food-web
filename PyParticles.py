...
class Particle:
    ...
    def __init__(self, obj, size, mass=1, val=0):
        x, y = obj
        self.val = val
        self.x = x
        self.y = y
    ...
    def attract(self, other):
        ...
        magnitude = 10
        self.accelerate((theta - 0.5 * math.pi, -magnitude * force / self.mass))
        other.accelerate((theta + 0.5 * math.pi, -magnitude * force / other.mass))
...
class Environment:
    ...
    def findParticle(self, obj):
        x, y = obj
        """ Returns any particle that occupies position x, y """

        for particle in self.particles:
            if math.hypot(particle.x - x, particle.y - y) <= 30:
                return particle
        return None
